...
glm::mat4 Camera::GetProjection() const {
    if (m_type == Type::PERSPECTIVE)
        ...
        float xFactor = (float)m_screenWidth /
                        (float)(m_screenWidth + m_screenHeight) *
                        abs(view[3][2]) * ratio_size_per_depth;
        float yFactor = (float)m_screenHeight /
                        (float)(m_screenWidth + m_screenHeight) *
                        abs(view[3][2]) * ratio_size_per_depth;
...
glm::mat4 Camera::CalculateRotateFromView(float x_offset,
                                          float y_offset) const {
    glm::mat4 rotation(1);
    glm::mat4 view = GetViewMatrix();
    rotation = glm::rotate(rotation, glm::radians((float)x_offset),
                           glm::vec3(view[1]));
    rotation = glm::rotate(rotation, glm::radians((float)-y_offset),
                           glm::vec3(view[0]));
    return rotation;
}

glm::mat4 Camera::CalculateTranslateFromView(float x_offset,
                                             float y_offset) const {
    ...
    glm::mat4 transform = glm::translate(
        glm::mat4(1.0), glm::vec3(xOffset_clip, yOffset_clip, 0));
