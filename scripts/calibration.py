...
def undistorImage(output_path, images, matrix, dist):
    ...
    (map1, map2) = cv2.initUndistortRectifyMap(
        matrix, dist, None, None, (width, height), cv2.CV_32FC1)
...
class ThermalVisualCalibrator():
    ...
    def getPoints(self, path, size, black_dot, verbose=False):
        ...
        for r in range(0, size[1]):
            ...
            for c in range(0, size[0]):
                x_index = size[0] - c
                if r % 2 == 0:
                    objp[c + r * size[0]] = [x_index * self.real_hstep +
                                             self.real_hstep / 2, y_index * self.real_vstep, 0]
                else:
                    objp[c + r * size[0]] = [x_index *
                                             self.real_hstep, y_index * self.real_vstep, 0]
        ...
        for idx, fname in enumerate(images):
            ...
            ret, corners = cv2.findCirclesGrid(
                gray, size, cv2.CALIB_CB_ASYMMETRIC_GRID, blobDetector, None)
    ...
    def startCalibration(self):
        ...
        self.thermal_images_list, self.thermal_world_pos, self.thermal_img_pos, shape = self.getPoints(
            self.thermal_path, self.calibration_pt_size, False)

        _, self.thermal_K, self.thermal_dist, thermal_rvecs, thermal_tvecs = cv2.calibrateCamera(
            self.thermal_world_pos, self.thermal_img_pos, shape, None, None)

        self.thermal_K_homo = utils.toHomogenousMatrix(self.thermal_K)
        for idx in range(0, len(thermal_rvecs)):
            transform = utils.rtToMatrix(
                thermal_rvecs[idx], thermal_tvecs[idx])
        ...
        self.visual_K_homo = utils.toHomogenousMatrix(self.visual_K)
        for idx in range(0, len(visual_rvecs)):
            transform = utils.rtToMatrix(
                visual_rvecs[idx], visual_tvecs[idx])
